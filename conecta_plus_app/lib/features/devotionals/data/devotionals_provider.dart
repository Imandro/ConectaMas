...
class DevotionalNotifier extends StateNotifier<DevotionalState> {
  final List<Struggle> _userStruggles;

  DevotionalNotifier(this._userStruggles) : super(DevotionalState()) {
    _init();
  }

  void _init() {
    // 1. Get relevant categories from user struggles
    final relevantCategories = _getCategoriesFromStruggles();

    // 2. Define all devotionals
    final allDevotionals = [
      Devotional(
        ...
        time: '3 min',
        image:
            'https://images.unsplash.com/photo-1474418397713-7ede21d49118?auto=format&fit=crop&q=80',
        bibleVerse:
        ...
        prayer:
            'Señor, hoy te entrego mi mente acelerada. No puedo con esto, pero Tú sí. Gracias porque no tengo que ser fuerte todo el tiempo. Recibo tu paz ahora. Amén.',
      ...
      Devotional(
        ...
        time: '4 min',
        image:
            'https://images.unsplash.com/photo-1499209974431-9dac3adaf471?auto=format&fit=crop&q=80',
        bibleVerse:
            '¿Quién de vosotros, por ansioso que esté, puede añadir una hora a su vida?',
        ...
        prayer:
            'Padre, perdóname por tratar de ser Dios. Suelto mi necesidad de controlarlo todo y descanso en tu sabiduría. Amén.',
        author: 'Mario Alvarez',
      ),
      Devotional(
        id: '1-3',
        ...
        prayer:
            'Señor, ayúdame a vivir en el presente. Tu gracia es suficiente para hoy. No temeré al mañana porque Tú ya estás allí. Amén.',
        author: 'Mario Alvarez',
      ),
      Devotional(
        id: '2',
        ...
        prayer:
            'Jesús, mi carne es débil, pero Tú eres mi fuerza. Limpia mi mente de imágenes que no te agradan. Ayúdame a ver la pureza no como una regla, sino como una forma de amarte más. Amén.',
        author: 'Mario Alvarez',
      ),
      Devotional(
        id: '3',
        ...
        prayer:
            'Padre, perdóname por buscar mi valor en cosas vacías. Gracias porque mi identidad está segura en Ti. Soy tuyo, y eso es suficiente. Amén.',
        author: 'Mario Alvarez',
      ),
      Devotional(
        id: '4',
        ...
        time: '3 min',
        image:
            'https://images.unsplash.com/photo-1518098268026-4e1877433665?auto=format&fit=crop&q=80',
        bibleVerse:
        ...
        prayer:
            'Dios, a veces me siento invisible. Gracias porque Tú siempre me ves. Llena este vacío con tu presencia real y tangible hoy. No estoy solo. Amén.',
        author: 'Mario Alvarez',
      ),
    ];

    // 3. Mark recommened ones
    final processedDevotionals = allDevotionals.map((dev) {
      if (relevantCategories.contains(dev.category)) {
        return dev.copyWith(isRecommended: true);
      }
      return dev.copyWith(isRecommended: false); // Default to false
    }).toList();

    // 4. Sort: Recommended first
    processedDevotionals.sort((a, b) {
    ...
    state = state.copyWith(
      devotionals: processedDevotionals,
    );
  }

  Set<String> _getCategoriesFromStruggles() {
    final categories = <String>{};
    for (var struggle in _userStruggles) {
      // Map struggle title to devotional category
      final title = struggle.title.toLowerCase();
      if (title.contains('ansied') || title.contains('estre')) {
        categories.add('Ansiedad');
      } else if (title.contains('porno') ||
          title.contains('mentira') ||
          title.contains('adicci')) {
        categories.add('Integridad');
      } else if (title.contains('soledad') || title.contains('triste')) {
        categories.add('Soledad');
      } else if (title.contains('autoestima') || title.contains('identidad')) {
        categories.add('Identidad');
      } else if (title.contains('relacion')) {
        categories.add('Relaciones');
      }
    }
    return categories;
  }
...
final devotionalsProvider =
  ...
  final struggles = ref.watch(struggleProvider).struggles;
  return DevotionalNotifier(struggles);
});
